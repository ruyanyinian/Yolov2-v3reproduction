...
class YOLOv3(nn.Module):
  def __init__(self,
               ...
               conf_thresh=0.001,
               nms_thresh=0.50,
               anchor_size=None, # anchor size [[32.64, 47.68], [50.24, 108.16], [126.72, 96.32], [78.4, 201.92], [178.24, 178.56], [129.6, 294.72], [331.84, 194.56], [227.84, 325.76], [365.44, 358.72]]
               hr=False):
    ...
    self.topk = 3000
    self.stride = [8, 16, 32]
    self.anchor_size = torch.tensor(anchor_size).view(3, len(anchor_size) // 3, 2) # 从(9,2) ==> (3, 3, 2)
    self.num_anchors = self.anchor_size.size(1)
    # 对原始输入(416,416)进行下采样,得到3种不同尺寸的feature map(52,52)(26,26)(13,13),
    # 然后对这些featuremap上的每一个点铺设3种不同尺寸的anchor, 然后把3种不同的featuremap信息整合到一起.
    # self.grid_cell(1, 52*52+26*26+13*13, 1, 2) = (1,3549,1,2)
    # self.stride_tensor(1, 52*52+26*26+13*13, 3, 2)
    # self.all_anchors_wh(1, 52*52+26*26+13*13, 3, 2)
  ...
  def create_grid(self, input_size):
    ...
    total_stride = []
    total_anchor_wh = []
    w, h = input_size, input_size # H=640, w=640
    '''
    这个循环遍历的是[8,16,32]这个list, 它在做以下的事情
    1. 对于原始图像输入(416,416)开始进行[8,16,32]倍的下采样,得到的是(52,52), (26,26)和(13,13)尺寸的feature map
    2. 创建针对feature map的每一个特征点的坐标, 比如说feature map是(52,52),也就是52*52=2704个特征点, 他的grid_xy=(1, 52*52=2704, 1, 2)
      a.比如说feature map的第一个特征点的坐标是(0,0), 那么对应的grid_xy[0, 0, 0] = (0,0), 那么对应的第二个点就是[0,1,0]=(1,0)
    3. stride_tensor是记录每一个特征点生成的3个anchor框下采样的倍率, 比如52*52尺寸的特征图是下采样了8倍,那么stride_tensor里面记录就是(8,8)的下采样
    4. anchor_wh(52*52,3,2)就是对每一个特征点开始进行铺anchor了, 每一个特征点是铺了3个尺寸的anchor,每一个anchor是由w,h组成的。
    5. 最终我们遍历完之后,再把这些特征图的anchor给整合起来。
    '''
    for ind, s in enumerate(self.stride): # self.stride = [8,16,32]
      # generate grid cells
      ws, hs = w // s, h // s  # 这一波的操作就是把输入进行了8,16和32倍的降采样, 然后输入变成了52,26,13
      grid_y, grid_x = torch.meshgrid([torch.arange(hs), torch.arange(ws)])
      grid_xy = torch.stack([grid_x, grid_y], dim=-1).float()
      grid_xy = grid_xy.view(1, hs*ws, 1, 2) # (1, 52*52, 1, 2)

      # generate stride tensor
      stride_tensor = torch.ones([1, hs*ws, self.num_anchors, 2]) * s # (1, 52*52,3,2)

      # generate anchor_wh tensor, anchor_size(3,3,2)
      anchor_wh = self.anchor_size[ind].repeat(hs*ws, 1, 1) # anchor_wh=(52*52,3,2) 现在就是在下采样的featuremap(52,52)(26,26)(13,13)中, 对每一个特征点开始铺anchor了

      total_grid_xy.append(grid_xy)
      total_stride.append(stride_tensor)
      total_anchor_wh.append(anchor_wh)

    total_grid_xy = torch.cat(total_grid_xy, dim=1).to(self.device) # (1, 52*52+26*26+13*13, 1, 2)
    total_stride = torch.cat(total_stride, dim=1).to(self.device) # (1, 52*52+26*26+13*13, 3, 2)
    total_anchor_wh = torch.cat(total_anchor_wh, dim=0).to(self.device).unsqueeze(0)  # (1, 52*52+26*26+13*13, 3, 2)
