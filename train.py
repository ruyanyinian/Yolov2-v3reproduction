...
def train():
  ...
  for epoch in range(args.start_epoch, max_epoch):
    ...
    # 因为把batch设置成了2, 所以images = torch.Size([2, 3, 416, 416]), 这里的第一个维度才是2
    # targets是一个list, 因为batch=2, 所以targets的list是两个元素, 每一个元素对应的是每一张图像
    # 每一个元素是一个torch.tensor, 比如说第一个元素的tensor shape=(3,5), 说明有三个待检测的物体
    # 每一个物体的前4个元素是用来描述这个物体的坐标值, 最后一个是类别值, 所以是5
    # 那么第二张图像对应的torchtensor是(1,5)对应的是一个待检测物体
    for iter_i, (images, targets) in enumerate(dataloader):
      ...
      # 这里的target里面的两个元素tensor(3,5)和tensor(1,5)变成了list of list, 然后外面再次包裹成一个list
      ...
      if local_rank == 0:
        ...
        if cur_map > best_map:
          ...
          torch.save(model_eval.state_dict(), os.path.join(path_to_save,
                                                           args.version + '_' + repr(epoch + 1) + '_' + str(
                                                             round(best_map, 2)) + '.pth')
                     )
        if args.tfboard:
          if args.dataset == 'voc':
