...
class VOCDetection(data.Dataset):
  ...
  def pull_item(self, index):
    # load an image
    img_id = self.ids[index] # index = 14151
    img = cv2.imread(self._imgpath % img_id)
    height, width, channels = img.shape

    # load a target
    target = ET.parse(self._annopath % img_id).getroot() # target: list of list,比如当前的target是有三个元素[[坐标信息, label值],[...],[]]
    ...
    if len(target) == 0:
      ...
      target = np.array(target) # 这个是(3, 5)
    # transform
